...
macro_rules! response {
    ($code:ident, $json:expr) => {
        || -> Result<HttpResponse, Box<dyn std::error::Error>> {
            ...
            let response = arthur_hugon_tools::security::public_encrypt(&key, response.as_bytes())?;
            let response = arthur_hugon_tools::reexports::encode(&serde_json::to_string(&response).unwrap());
...
macro_rules! encrypt {
    ($key:expr, $e:expr) => {
        |k: &[u8], m: String| -> Result<String, Box<dyn std::error::Error>> {
            let key = arthur_hugon_tools::reexports::rsa::Rsa::public_key_from_pem(k)?;
            let r = arthur_hugon_tools::security::public_encrypt(&key, m.as_bytes())?;
            let r = arthur_hugon_tools::reexports::encode(&serde_json::to_string(&r).unwrap());
            Ok(r)
        }($key, $e)
        .map_err(|_| ())?
    };
...
macro_rules! decrypt {
    ($e:expr) => {
        |m: String| -> Result<String, Box<dyn std::error::Error>> {
            let key = arthur_hugon_tools::reexports::rsa::Rsa::private_key_from_pem(
                crate::routes::PRIVATE,
            )?;
            let r = serde_json::from_str::<arthur_hugon_tools::security::Tomb>(
                String::from_utf8(base64::decode(&m)?)?.as_str(),
            )?;
            let r = arthur_hugon_tools::security::private_decrypt(&key, r)?;
            Ok(r.value)
        }($e)
        .map_err(|_| ())?
    };
